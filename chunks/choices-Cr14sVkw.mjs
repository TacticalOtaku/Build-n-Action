//#region src/core/blueprint/normalize.ts
/**
* Blueprints saved before "only this item" became a tick on the condition carried it as a check.
* In an all-of condition the two mean the same, so the check becomes the tick; any-of conditions keep it,
* since there the check was one alternative among others. Returns the same blueprint when nothing changes.
*/
function foldThisItemChecks(blueprint) {
	let changed = false;
	const nodes = blueprint.nodes.map((node) => {
		if (node.kind !== "condition" || node.data.mode === "any" || !Array.isArray(node.data.checks)) return node;
		const checks = node.data.checks;
		const kept = checks.filter((check) => check.type !== "thisItem");
		if (kept.length === checks.length) return node;
		changed = true;
		return {
			...node,
			data: {
...
/** Events that start without a roll: turns, regions, rests. */
var TRIGGER_EVENTS = [
	"turnStart",
	"turnEnd",
	"regionEnter",
...
/** Internal events of continuations: what follows a result's own outputs. They have no palette entry. */
var CONTINUATION_EVENTS = ["afterResult", "demandedSave"];
/** The internal event a continuation runs under, by the result pin it leaves from. */
var CONTINUATION_EVENT_BY_PIN = {
	then: "afterResult",
	succeeded: "demandedSave",
	failed: "demandedSave"
};
var NODE_KINDS = [
	"event",
...
//#endregion
//#region src/core/counters/counters.ts
var COUNTER_RESETS = [
	"manual",
	"shortRest",
	"longRest",
...
/** A counter name as stored: trimmed, lower case, runs of spaces and dots as "_". */
function counterName(raw) {
	return raw.trim().toLocaleLowerCase().replace(/[\s.]+/g, "_");
}
function counterKey(scope, name, blueprintId) {
	return scope === "blueprint" ? `${blueprintId}:${name}` : name;
}
function readCounterDef(value, name = "") {
	const record = asRecord(value);
	const reset = asString(record.reset, "manual");
	const min = asNumber(record.min) ?? DEFAULT_COUNTER.min;
	const max = asNumber(record.max);
	return {
		name: counterName(asString(record.name, name)),
		min,
		max: max === null ? null : Math.max(max, min),
		initial: asNumber(record.initial) ?? DEFAULT_COUNTER.initial,
		reset: COUNTER_RESETS.includes(reset) ? reset : "manual"
	};
}
function readCounterStore(value) {
	const record = asRecord(value);
	const values = {};
	for (const [key, entry] of Object.entries(asRecord(record.values))) {
		const number = asNumber(entry);
		if (number !== null) values[key] = number;
	}
	const defs = {};
	for (const [name, entry] of Object.entries(asRecord(record.defs))) defs[counterName(name)] = readCounterDef(entry, name);
	return {
		values,
		defs
	};
}
function counterDef(store, scope, name, blueprint) {
	const key = counterName(name);
	return (scope === "blueprint" ? (blueprint.counters ?? []).map((entry) => readCounterDef(entry)).find((entry) => entry.name === key) : store.defs[key]) ?? {
		name: key,
		...DEFAULT_COUNTER
	};
}
function counterValue(store, scope, name, blueprint) {
	const key = counterName(name);
	return store.values[counterKey(scope, key, blueprint.id)] ?? counterDef(store, scope, key, blueprint).initial;
}
/** Formulas name counters without a kind: the blueprint's own when it has one by that name, else the shared one. */
function formulaScope(store, name, blueprint) {
	const key = counterName(name);
	return (blueprint.counters ?? []).some((entry) => counterName(entry.name) === key) || counterKey("blueprint", key, blueprint.id) in store.values ? "blueprint" : "actor";
}
function clamp(value, def) {
	const floored = Math.max(def.min, value);
	return def.max === null ? floored : Math.min(def.max, floored);
}
/** The value after an action ("add", "subtract", "set", "reset"), rounded down to whole numbers and kept within limits. */
function nextCounterValue(current, action, amount, def) {
	const step = Math.trunc(amount);
	switch (action) {
		case "add": return clamp(current + step, def);
		case "subtract": return clamp(current - step, def);
		case "set": return clamp(step, def);
		default: return clamp(def.initial, def);
	}
}
/** Short-rest counters reset on both rests; long-rest ones on long rests; turn ones at the start of a turn. */
function resetsOn(event, reset) {
	if (event === "turnStart") return reset === "turnStart";
	if (event === "shortRest") return reset === "shortRest";
	return reset === "shortRest" || reset === "longRest";
}
/** The values an event resets, by key: shared counters of the actor and those of its blueprints. */
function counterResets(store, blueprints, event) {
	const resets = {};
	for (const def of Object.values(store.defs)) if (resetsOn(event, def.reset)) resets[counterKey("actor", def.name, "")] = clamp(def.initial, def);
	for (const blueprint of blueprints) for (const def of (blueprint.counters ?? []).map((entry) => readCounterDef(entry))) if (resetsOn(event, def.reset)) resets[counterKey("blueprint", def.name, blueprint.id)] = clamp(def.initial, def);
	return resets;
}
var TARGET_REF = /@target\.counters\.([\p{L}\p{N}_]+)/gu;
var SELF_REF = /@counters\.([\p{L}\p{N}_]+)/gu;
/** Replace @target.counters.<name> and @counters.<name> with numbers; the target's are 0 without a target. */
function substituteCounters(text, self, target) {
	if (!text.includes("counters.")) return text;
	return text.replace(TARGET_REF, (_match, name) => String(target ? target(counterName(name)) : 0)).replace(SELF_REF, (_match, name) => String(self(counterName(name))));
}
/** Apply a string replacement to every string inside plain data (records and arrays). */
function substituteDeep(value, replace) {
	if (typeof value === "string") return replace(value);
	if (Array.isArray(value)) return value.map((entry) => substituteDeep(entry, replace));
	if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, substituteDeep(entry, replace)]));
	return value;
}
function nodeCounters(node) {
	if (node.kind === "result" && node.type === "counter") return [node.data];
	if (node.kind !== "condition") return [];
	return (Array.isArray(node.data.checks) ? node.data.checks : []).filter((check) => check.type === "counter").map((check) => check.data);
}
/** "My" counters a blueprint uses, in graph order, each once: from counter nodes and checks, then from formulas. */
function usedCounters(blueprint) {
	const found = /* @__PURE__ */ new Map();
	const add = (name, scope) => {
		const key = counterName(name);
		if (key && !found.has(`${scope}:${key}`)) found.set(`${scope}:${key}`, {
			name: key,
			scope
		});
	};
	for (const node of blueprint.nodes) for (const data of nodeCounters(node)) if (asString(data.who ?? data.whose, "self") === "self") add(asString(data.name), asString(data.scope) === "blueprint" ? "blueprint" : "actor");
	const text = JSON.stringify(blueprint.nodes.map((node) => node.data));
	for (const match of text.matchAll(/(?<!target\.)@counters\.([\p{L}\p{N}_]+)/gu)) {
		const name = counterName(match[1] ?? "");
		add(name, formulaScope(EMPTY_STORE, name, blueprint));
	}
	return [...found.values()];
}
//#endregion
...
var STATIC = {
	...
	turnReachMode: choices("reachMode", ["self", "aura"]),
	regionTokens: choices("regionTokens", [
		"any",
		"friendly",
		"hostile"
	]),
	...
	saveWho: choices("saveWho", [
		"target",
		"self",
		"aura"
	]),
	saveDcMode: choices("saveDcMode", ["formula", "spell"]),
	rollMode: choices("rollMode", [
		"normal",
		"advantage",
		"disadvantage"
	]),
	counterScope: choices("counterScope", ["actor", "blueprint"]),
	counterAction: choices("counterAction", [
		"add",
		"subtract",
		"set",
		"reset"
	]),
	counterOperator: choices("counterOperator", [
		"EQ",
		"NE",
		"LT",
		"LE",
	...
	counterReset: choices("counterReset", [
		"manual",
		"shortRest",
		"longRest",
		"turnStart"
...
var CONDITION_NODE_FIELDS = [{
	...
	key: "thisItem",
	kind: "toggle",
	label: "BNA.Field.conditionThisItem.label",
	hint: "BNA.Field.conditionThisItem.hint",
	default: false
}];
...
var counterCheck = {
	...
	fields: [
		...
		},
		{
			key: "scope",
			kind: "select",
			label: "BNA.Field.counterScope.label",
			hint: "BNA.Field.counterScope.hint",
			default: "actor",
			choices: { static: STATIC.counterScope }
		},
		{
			key: "whose",
			kind: "select",
			label: "BNA.Check.counter.Field.whose",
			default: "self",
			choices: { static: STATIC.who }
		},
		{
			key: "operator",
			kind: "select",
			label: "BNA.Check.counter.Field.operator",
			default: "GE",
			choices: { static: STATIC.counterOperator }
		},
		{
			key: "value",
			kind: "formula",
			label: "BNA.Check.counter.Field.value",
			default: "1"
		}
	],
	phrase: (data, fmt) => fmt.t(`BNA.Check.counter.Phrase.${asString(data.whose) === "target" ? "target" : "self"}`, {
		name: counterName(asString(data.name)) || fmt.t("BNA.Phrase.Unset"),
		operator: COUNTER_SYMBOLS[asString(data.operator, "GE")] ?? "=",
		value: asString(data.value).trim() || "0"
	})
};
...
function reachField(modes, text = "reach") {
	return {
		key: "reach",
		kind: "group",
		label: `BNA.Field.${text}.label`,
		hint: `BNA.Field.${text}.hint`,
		fields: [
			{
				key: "mode",
				kind: "select",
				label: "BNA.Field.reachMode.label",
				hint: `BNA.Field.${text}Mode.hint`,
				default: "self",
				choices: { static: modes }
			},
			{
				key: "range",
				kind: "formula",
...
var REACH_FIELD = reachField(STATIC.reachMode);
/** Turn events: my turn, or the turn of a creature in my aura. */
var TURN_REACH = reachField(STATIC.turnReachMode, "turnReach");
/** Region events: which tokens count, by their disposition. */
var REGION_TOKENS = {
	key: "tokens",
	kind: "select",
	label: "BNA.Field.regionTokens.label",
	hint: "BNA.Field.regionTokens.hint",
	default: "any",
	choices: { static: STATIC.regionTokens }
};
function trigger(type, icon, fields) {
	return {
		kind: "event",
		type,
		phase: "trigger",
...
var EVENTS = [
	...
	trigger("turnStart", "play", [TURN_REACH]),
	trigger("turnEnd", "square", [TURN_REACH]),
	trigger("regionEnter", "log-in", [REGION_TOKENS]),
	trigger("regionExit", "log-out", [REGION_TOKENS]),
	trigger("shortRest", "sun", []),
	trigger("longRest", "moon", []),
	{
		kind: "event",
		type: "signal",
		phase: "signal",
...
/** Events without a roll: results that change a roll do not fit. */
var TRIGGERS = TRIGGER_EVENTS;
/** After a result's "And then" output: the roll is already decided. */
var AFTER_RESULT = ["afterResult"];
/** After a demanded save's outcome outputs. */
var AFTER_SAVE = ["demandedSave"];
...
var COMMON_RESULT_FIELDS = [
	...
	},
	{
		key: "then",
		kind: "toggle",
		label: "BNA.Field.then.label",
		hint: "BNA.Field.then.hint",
		default: false
	}
];
var rollBonus = {
...
var SAVE_WHO = { static: STATIC.saveWho };
		...
		fields: [
			{
				key: "ability",
				kind: "select",
				label: "BNA.Result.demandSave.Field.ability",
				default: "con",
				choices: ABILITIES
			},
			{
				key: "dcMode",
				kind: "select",
				label: "BNA.Result.demandSave.Field.dcMode",
				hint: "BNA.Result.demandSave.Field.dcModeHint",
				default: "formula",
				choices: { static: STATIC.saveDcMode }
			},
			{
				...formula("dc", "BNA.Result.demandSave.Field.dc", "BNA.Result.demandSave.Field.dcHint", "13"),
				visibleWhen: {
					key: "dcMode",
					in: ["formula"]
				}
			},
			{
				key: "who",
				kind: "select",
				label: "BNA.Result.demandSave.Field.who",
				default: "target",
				choices: SAVE_WHO
			},
			{
				...formula("range", "BNA.Field.reachRange.label", "BNA.Field.reachRange.hint", "10"),
				visibleWhen: AURA
			},
			{
				key: "disposition",
			...
			},
			{
				key: "advantage",
				kind: "select",
				label: "BNA.Result.demandSave.Field.advantage",
				default: "normal",
				choices: { static: STATIC.rollMode }
			}
		],
		flavor: () => [],
		phrase: (data, fmt) => {
			const who = asString(data.who, "target");
			const ability = fmt.choice(asString(data.ability, "con"), ABILITIES) || fmt.t("BNA.Phrase.Unset");
			const dc = asString(data.dcMode) === "spell" ? fmt.t("BNA.Result.demandSave.DcSpell") : fmt.t("BNA.Result.demandSave.DcFormula", { formula: asString(data.dc).trim() || "10" });
			const advantage = asString(data.advantage, "normal");
			const text = fmt.t(`BNA.Result.demandSave.Phrase.${who}`, {
				ability,
				dc,
				range: asString(data.range).trim() || "10"
			});
			return advantage === "normal" ? text : fmt.t(`BNA.Result.demandSave.With.${advantage}`, { text });
		}
	},
	{
		kind: "result",
		...
		fields: [
			{
				key: "name",
				kind: "text",
				label: "BNA.Result.counter.Field.name",
				hint: "BNA.Result.counter.Field.nameHint",
				default: "",
				suggest: "counters"
			},
			{
			...
			},
			{
				key: "action",
				kind: "select",
				label: "BNA.Result.counter.Field.action",
				default: "add",
				choices: { static: STATIC.counterAction }
			},
			{
				...formula("amount", "BNA.Result.counter.Field.amount", "BNA.Result.counter.Field.amountHint", "1"),
				visibleWhen: {
					key: "action",
					in: [
						"add",
			...
			whoField("counter", "self")
		],
		flavor: () => [],
		phrase: (data, fmt) => {
			const who = asString(data.who) === "target" ? "target" : "self";
			return fmt.t(`BNA.Result.counter.Phrase.${who}.${asString(data.action, "add")}`, {
				name: counterName(asString(data.name)) || fmt.t("BNA.Phrase.Unset"),
				amount: asString(data.amount).trim() || "1"
			});
		}
...
var THEN_PINS = ["then"];
/** The outputs a node has: an event's outcomes, a condition's yes/no, a result's "And then" while it is on. */
function outputPins(node) {
	switch (node.kind) {
		case "event": return getEvent(node.type)?.pins ?? [];
		case "condition": return node.type === "condition" ? CONDITION_PINS : [];
		case "result": {
			const definition = getResult(node.type);
			if (!definition) return [];
			if (definition.pins?.length) return definition.pins;
			return asRecord(node.data.common).then === true ? THEN_PINS : [];
		}
	}
}
...
function canConnect(graph, request) {
	...
	const pins = outputPins(source);
	if (source.kind === "result" && !pins.length) return reject("fromResult");
	if (!pins.includes(request.sourcePin)) return reject("unknownPin");
...
function validateBlueprintSource(input) {
	...
	if (input.counters !== void 0) {
		if (!Array.isArray(input.counters)) report("counters", "invalidField");
		else input.counters.forEach((entry, index) => {
			if (!(isRecord(entry) && typeof entry.name === "string" && !!entry.name.trim() && typeof entry.min === "number" && (entry.max === null || typeof entry.max === "number") && typeof entry.initial === "number" && COUNTER_RESETS.includes(entry.reset))) report(`counters.${index}`, "invalidField");
		});
	}
	...
	rawEdges.forEach((edge, index) => {
		...
		if (target.kind === "event" || edge.source === edge.target || edge.targetPin !== "in") return report(path, "invalidDirection");
		if (!outputPins(source).includes(edge.sourcePin)) return report(`${path}.sourcePin`, "invalidPin");
...
function createBlueprint(init = {}) {
	return {
		...
		edges: init.edges ?? [],
		flags: init.flags ?? {},
		...init.counters ? { counters: init.counters.map((counter) => ({ ...counter })) } : {}
	};
}
	...
	for (const [key, value] of Object.entries(asRecord(filtersRaw))) {
		...
		switch (key) {
			...
			case "healthPercentages": {
				...
				if (threshold !== null && type !== null) add(key, {
					value: threshold,
					type: type === 1 ? "atLeast" : "atMost"
				});
				break;
			}
			...
			case "tokenSizes": {
				...
				const type = asNumber(record.type);
				if (size !== null && size > 0 && type !== null) add(key, {
					size,
					type: type === 1 ? "atMost" : "atLeast",
...
function convertLegacyBonus(raw, carrier, options = {}) {
	...
	const onlyThisItem = asBoolean(source.exclusive);
	const needsCondition = checks.length > 0 || onlyThisItem;
	const bonuses = asRecord(source.bonuses);
	...
	const column = needsCondition ? COLUMN * 2 : COLUMN;
...
function readBlueprints(scope, carrier) {
	...
	for (const raw of storedEntries(flags.blueprints)) {
		...
		blueprints.push(foldThisItemChecks(result.value));
	}
...
/**
* Every route from an event (or, with `from: "results"`, from a result's output) to a result. Conditions reached through several wires yield one
...
*/
function enumeratePaths(graph, from = "events") {
	...
	for (const node of graph.nodes) {
		if (node.kind !== (from === "events" ? "event" : "result")) continue;
		const pins = outputPins(node);
		for (const edge of outgoing.get(node.id) ?? []) {
			if (node.kind === "result" && !pins.includes(edge.sourcePin)) continue;
			visit(edge.target, {
				eventNodeId: node.id,
				eventPin: edge.sourcePin
			}, [], /* @__PURE__ */ new Set([node.id]));
		}
	}
	return paths;
}
...
function compileBlueprint(source) {
	...
	const build = (path, eventType) => {
		...
		for (const guard of path.guards) {
			...
			guards.push({
				...
				thisItem: node.data.thisItem === true
			});
			previous = node.id;
	...
	const paths = enumeratePaths(source).flatMap((path) => build(path, nodes.get(path.eventNodeId)?.type) ?? []);
	const continuations = enumeratePaths(source, "results").flatMap((path) => build(path, CONTINUATION_EVENT_BY_PIN[path.eventPin]) ?? []);
	const byEvent = groupBy(paths, (path) => path.eventType);
	const byResult = groupBy(continuations, (path) => path.event.id);
	const compiled = {
		source,
		paths,
		pathsFor: (event) => byEvent.get(event) ?? [],
		continuations,
		continuationsOf: (nodeId) => byResult.get(nodeId) ?? []
	};
...
var BlueprintEditorApp = class extends foundry.applications.api.ApplicationV2 {
	...
	async _onFirstRender() {
		...
		if (!content) return;
		try {
			const { mountEditor } = await import("./editor-DHJRYAbj.mjs");
...
var EVALUATORS = {
	...
	counter: (data, _facts, env) => {
		const name = counterName(asString(data.name));
		if (!name) return passed("empty");
		const scope = asString(data.scope) === "blueprint" ? "blueprint" : "actor";
		const whose = asString(data.whose) === "target" ? "target" : "self";
		const operator = asString(data.operator, "EQ");
		const actual = env.counter?.(name, scope, whose) ?? 0;
		const wanted = env.evaluateNumber(asString(data.value).trim() || "0");
		const shown = {
			left: `${name} = ${actual}`,
			operator: OPERATOR_SYMBOLS[operator] ?? "=",
			right: String(wanted ?? "—")
		};
		if (wanted === null || !compare(operator, actual, wanted)) return failed("comparisonFailed", shown);
		return passed("matched", { actual });
	},
...
function evaluateGuard(guard, facts, env, cache, nodes) {
	...
	if (guard.thisItem) {
		const outcome = evaluateCheck("thisItem", {}, facts, env);
		checks.unshift({
			checkId: "thisItem",
			type: "thisItem",
			outcome
		});
		value = outcome.pass && value;
	}
...
function runBlueprint(blueprint, trigger, facts, env) {
	...
	const paths = trigger.from ? compiled.continuationsOf(trigger.from) : compiled.pathsFor(trigger.event);
	if (trigger.from) run.nodes[trigger.from] = { status: "passed" };
	for (const path of paths) {
...
/** The pin a result's continuation leaves from when followed in a test run, or null when it has none. */
function followedPin(intent, outcome) {
	if (getResult(intent.type)?.pins?.length) return outcome;
	return intent.common.then ? "then" : null;
}
/** Run a blueprint, then follow each result's continuation ("And then", a demanded save's outcome) as if it applied. */
function runThrough(blueprint, trigger, facts, env, options = {}) {
	const maxDepth = options.maxDepth ?? 8;
	const outcome = options.outcome ?? "failed";
	const compiled = "paths" in blueprint ? blueprint : compileBlueprint(blueprint);
	const run = runBlueprint(compiled, trigger, facts, env);
	const through = {
		...run,
		intents: [...run.intents],
		nodes: { ...run.nodes },
	...
	let wave = run.intents;
	for (let depth = 0; wave.length && depth < maxDepth; depth++) {
		const next = [];
		for (const intent of wave) {
			const pin = followedPin(intent, outcome);
			const event = pin ? CONTINUATION_EVENT_BY_PIN[pin] : void 0;
			if (!pin || !event || !compiled.continuationsOf(intent.nodeId).length) continue;
			if (intent.common.optional) through.assumedChoice = true;
			const more = runBlueprint(compiled, {
				event,
				pins: [pin],
				from: intent.nodeId
			}, facts, env);
			for (const [id, trace] of Object.entries(more.nodes)) if (trace.status !== "skipped") through.nodes[id] = trace;
			for (const edge of more.edges) if (!through.edges.includes(edge)) through.edges.push(edge);
			through.intents.push(...more.intents);
			next.push(...more.intents);
		}
		wave = next;
	}
	return through;
...
*/
function runWithSignals(entries, trigger, facts, options = {}) {
	const maxDepth = options.maxDepth ?? 8;
	const result = {
		runs: [],
		intents: [],
	...
	let wave = [{
		trigger,
		receivers: options.starters ?? entries
	}];
...
//#endregion
//#region src/core/graph/upstream.ts
/**
* Event types whose chains can reach a node, found by walking wires backwards. Used to offer
* only compatible results, hide fields that do not apply, and flag irrelevant checks.
*/
function upstreamEvents(graph, nodeId) {
	const nodes = new Map(graph.nodes.map((node) => [node.id, node]));
	const start = nodes.get(nodeId);
	if (!start) return [];
	if (start.kind === "event") return [start.type];
	...
	for (let current = queue.shift(); current !== void 0; current = queue.shift()) {
		if (seen.has(current)) continue;
		seen.add(current);
...
/**
* The events a result placed after this node would run under. A result on the way stands for the
* continuation of the pin the route leaves it from ("afterResult" for "And then", "demandedSave" for a
* save's outcomes) and hides what came before it; conditions pass the search on.
*/
function resultEvents(graph, nodeId, pin) {
	const nodes = new Map(graph.nodes.map((node) => [node.id, node]));
	const incoming = /* @__PURE__ */ new Map();
	for (const edge of graph.edges) {
		const list = incoming.get(edge.target) ?? [];
	...
	for (let current = queue.shift(); current !== void 0; current = queue.shift()) {
		...
		if (!node) continue;
		if (node.kind === "event") add(node.type);
		else if (node.kind === "result") add(current.pin && CONTINUATION_EVENT_BY_PIN[current.pin] || "afterResult");
		else queue.push(...incoming.get(current.id) ?? []);
	}
	return found;
}
//#endregion
...
var LEVELS = {
	...
	"item-tick-off-item": "info",
	"optional-on-trigger": "warning",
	"region-event-off-region": "warning",
	"counter-unknown": "warning",
	"migrated": "info"
};
...
function lintBlueprint(blueprint, env = {}) {
	...
	for (const path of [...compiled.paths, ...compiled.continuations]) {
		...
		if ((TRIGGER_EVENTS.includes(eventType) || CONTINUATION_EVENTS.includes(eventType)) && asRecord(path.result.data.common).optional === true && once(`optional:${path.result.id}`)) report("optional-on-trigger", { nodeId: path.result.id });
		const origins = path.event.kind === "result" ? upstreamEvents(blueprint, path.event.id) : [eventType];
		for (const guard of path.guards) for (const check of guard.checks) {
			const definition = getCheck(check.type);
			if (definition && origins.length && !origins.some((event) => definition.relevantTo.includes(event)) && once(`check:${check.id}`)) report("irrelevant-check", {
				nodeId: guard.node.id,
				checkId: check.id,
				values: { event: origins[0] ?? eventType }
			});
		}
	}
	...
	if (env.counterNames) {
		const known = /* @__PURE__ */ new Set([...env.counterNames, ...(blueprint.counters ?? []).map((def) => counterName(def.name))]);
		const unknown = (data) => {
			const name = counterName(asString(data.name));
			return !!name && !known.has(name);
		};
		for (const node of blueprint.nodes) {
			if (node.kind === "result" && node.type === "counter" && unknown(node.data)) report("counter-unknown", {
				nodeId: node.id,
				values: { name: counterName(asString(node.data.name)) }
			});
			if (node.kind !== "condition") continue;
			for (const check of Array.isArray(node.data.checks) ? node.data.checks : []) if (check.type === "counter" && unknown(check.data)) report("counter-unknown", {
				nodeId: node.id,
				checkId: check.id,
				values: { name: counterName(asString(check.data.name)) }
			});
		}
	}
	if (env.regionCarrier === false) {
		for (const node of blueprint.nodes) if (node.kind === "event" && (node.type === "regionEnter" || node.type === "regionExit")) report("region-event-off-region", { nodeId: node.id });
	}
	if (env.itemCarrier === false) {
		for (const node of blueprint.nodes) if (node.kind === "condition" && node.data.thisItem === true) report("item-tick-off-item", { nodeId: node.id });
	}
	for (const node of blueprint.nodes) {
...
/** Region events: whether the token that entered or left counts, by its own disposition. */
function regionTokensOutcome(wanted, rollerDisposition) {
	if (wanted === "friendly") return rollerDisposition === DISPOSITION.FRIENDLY ? passed("matched", { actual: "friendly" }) : outOfReach("disposition");
	if (wanted === "hostile") return rollerDisposition === DISPOSITION.HOSTILE ? passed("matched", { actual: "hostile" }) : outOfReach("disposition");
	return passed("matched", { actual: "any" });
}
//#endregion
...
async function confirmDelete(document) {
	return !!await call$1(document, "deleteDialog");
}
...
async function payCost(cost, option, actor, info) {
	...
	switch (cost.type) {
		...
		case "slots":
			...
			return true;
		case "health":
			await call$1(actor, "applyDamage", option.amount);
			return true;
		...
		case "currency":
			...
			return true;
		case "hitdice":
			await call$1(actor, "updateEmbeddedDocuments", "Item", hitDiceSpend(classHitDice(actor), cost.subtype, option.amount).map((entry) => ({
				_id: entry.id,
...
async function executeOperation(op) {
	...
	switch (op.kind) {
		...
		case "rider":
			...
			return;
		case "counter":
			await actor.update({ [`flags.${MODULE_SCOPE}.counters.values.${op.key}`]: op.value });
			return;
	}
}
//#endregion
...
/** Runs a trigger another client sent; set by triggers.ts, which imports this module. */
var receiveTrigger = null;
function setTriggerReceiver(receiver) {
	receiveTrigger = receiver;
}
/** Handles demanded-save messages; set by saves.ts. */
var receiveSave = null;
function setSaveReceiver(receiver) {
	receiveSave = receiver;
}
/** Demanded saves: requests, questions, answers and cancellations between the asking client, the GM and the player. */
...
async function onMessage(message) {
	...
	if (typeof type === "string" && type.startsWith("save")) {
		receiveSave?.(message);
		return;
	}
	if (type === "trigger") {
		if (!game.user.isGM || read(message, "gm") !== game.user.id) return;
		const event = asString(read(message, "event"));
		if (!TRIGGER_EVENTS.includes(event)) return;
		await receiveTrigger?.(event, asString(read(message, "actorUuid")), asString(read(message, "key")));
		return;
	}
...
/** Ask the active GM to run a trigger (a rest finished on a player's client). */
function sendTrigger(gm, event, actorUuid, key) {
	game.socket.emit(CHANNEL, {
		type: "trigger",
		gm,
		event,
...
//#endregion
//#region src/foundry/counters.ts
function counterStore(actor) {
	return actor ? readCounterStore(read(actor, `flags.${MODULE_SCOPE}.counters`)) : EMPTY_STORE;
}
/** Every blueprint on an actor's documents: the actor, its items and its effects. */
function actorBlueprints(actor) {
	return [
		actor,
		...listOf(read(actor, "items")),
		...listOf(read(actor, "effects"))
	].filter(isDocument).flatMap((document) => documentBlueprints(document).blueprints);
}
/**
* Counter lookups for one blueprint: "my" counters on `owner`, the target's on `target`. `overrides` (the Test tab)
* replace my values by "<scope>:<name>".
*/
function counterLookups(owner, target, blueprint, overrides = {}) {
	const mine = counterStore(owner);
	const theirs = counterStore(target);
	const own = (name, scope) => overrides[`${scope}:${name}`] ?? counterValue(mine, scope, name, blueprint);
	return {
		self: (name) => own(name, formulaScope(mine, name, blueprint)),
		target: target ? (name) => counterValue(theirs, formulaScope(theirs, name, blueprint), name, blueprint) : null,
		value: (name, scope, whose) => whose === "target" ? counterValue(theirs, scope, name, blueprint) : own(name, scope)
	};
}
/** Results with every @counters.* and @target.counters.* reference replaced by the value now. */
function resolveCounterIntents(intents, sources, roller) {
	return intents.map((intent) => {
		const info = sources.get(intent.entry);
		if (!info || !JSON.stringify([intent.data, intent.common]).includes("counters.")) return intent;
		const lookups = counterLookups(info.owner ?? roller.actor, roller.target?.actor ?? null, info.blueprint);
		const replace = (text) => substituteCounters(text, lookups.self, lookups.target);
		return {
			...intent,
			data: substituteDeep(intent.data, replace),
			common: substituteDeep(intent.common, replace)
		};
	});
}
/** Reset the actor's counters that the event resets (GM side, with the triggers). */
async function resetCounters(actor, event) {
	const resets = counterResets(counterStore(actor), actorBlueprints(actor), event);
	const updates = Object.fromEntries(Object.entries(resets).map(([key, value]) => [`flags.${MODULE_SCOPE}.counters.values.${key}`, value]));
	if (Object.keys(updates).length) await actor.update(updates);
}
/** The values of the counters a blueprint uses, on an actor. */
function counterRows(actor, blueprint) {
	if (!actor) return [];
	const store = counterStore(actor);
	return usedCounters(blueprint).map((used) => ({
		...used,
		value: counterValue(store, used.scope, used.name, blueprint)
	}));
}
/** The editor's view of counters for a carrier: values on its owner, definitions, resets. */
function countersHost(carrier) {
	const owner = () => ownerActor(carrier);
	return {
		rows: (blueprint) => counterRows(owner(), blueprint),
		names: (blueprint) => [.../* @__PURE__ */ new Set([...Object.keys(counterStore(owner()).defs), ...(blueprint.counters ?? []).map((def) => counterName(def.name))])],
		reset: async (blueprint, row) => {
			const actor = owner();
			if (!actor) return;
			const def = counterDef(counterStore(actor), row.scope, row.name, blueprint);
			await performOperations([{
				kind: "counter",
				actor: actor.uuid,
				key: counterKey(row.scope, row.name, blueprint.id),
				value: def.initial
			}]);
		},
		actorDefs: () => {
			const actor = owner();
			return actor ? Object.values(counterStore(actor).defs) : null;
		},
		saveActorDefs: async (defs) => {
			const actor = owner();
			if (!actor) return;
			await actor.update({ [`flags.${MODULE_SCOPE}.counters.-=defs`]: null });
			await actor.update({ [`flags.${MODULE_SCOPE}.counters.defs`]: Object.fromEntries(defs.map((def) => [def.name, { ...def }])) });
		},
		subscribe: (listener) => {
			const id = Hooks.on("updateActor", (actor, changes) => {
				if (actor === owner() && JSON.stringify(changes).includes("counters")) listener();
			});
...
function addEntry(collected, info, candidate, roller, auras, overrides = {}) {
	const { blueprint } = info;
	const context = sourceContext(candidate, info, roller, auras);
	const counters = counterLookups(info.owner ?? roller.actor, roller.target?.actor ?? null, blueprint, overrides);
	const withCounters = (text) => substituteCounters(text, counters.self, counters.target);
	const env = {
		evaluateNumber: (formula) => evaluateNumber(withCounters(formula), roller.rollData),
		replaceData: (text) => replaceData(withCounters(text), roller.rollData),
		...
		reach: (node) => {
			...
			return typeof data.tokens === "string" ? regionTokensOutcome(data.tokens, context.rollerDisposition) : reachOutcome(data.reach, context);
		},
...
function collect(roller, event, activation = null, options = {}) {
	const collected = {
		entries: [],
	...
	for (const rider of options.only ? [] : activeRiders(roller.actor, event, activation)) {
...
*/
function entryFor$1(document, blueprint, roller, overrides = {}) {
	...
	addEntry(collected, {
		...
		kind,
		sourceToken
	}, roller, /* @__PURE__ */ new Map(), overrides);
...
var afterRolls = /* @__PURE__ */ new WeakMap();
function rememberAfterRoll(config, after) {
	if (config && typeof config === "object") afterRolls.set(config, after);
}
/** The roll's AfterRoll, forgotten as it is taken so it continues once. */
function takeAfterRoll(config) {
	if (!config || typeof config !== "object") return null;
	const found = afterRolls.get(config) ?? null;
	afterRolls.delete(config);
	return found;
}
var records = [];
...
function evaluateEvent(event, roller, pins = ["out"], activation = null, options = {}) {
	const started = performance.now();
	const collected = collect(roller, event, activation, options);
	...
	chain.intents = resolveCounterIntents(chain.intents, collected.sources, roller);
...
function simulate(carrier, blueprint, request) {
	...
	const { entry, suppressed } = entryFor$1(carrier, blueprint, roller, request.counters);
	const facts = buildRollFacts({
		event: request.event,
		actor,
		token: roller.token?.document ?? null,
	...
	if (TRIGGER_EVENTS.includes(request.event)) notes.push("trigger");
	const run = runThrough(entry.compiled, trigger, facts, entry.env, { outcome: request.saveOutcome });
	if (run.assumedChoice) notes.push("chosen");
	return {
		run,
		notes
	};
	...
	return {
		...
		lintEnv: (blueprint) => {
			...
			return {
				...
				hasPath: (path) => ROLL_ROOTS.some((root) => path.startsWith(root)) || path.startsWith("counters.") || rollPaths().known.has(path),
				signalsSent: sent,
				signalsReceived: received,
				itemCarrier: carrierItem(carrier) !== null,
				counterNames: new Set(Object.keys(counterStore(ownerActor(carrier)).defs)),
				regionCarrier: carrier.documentName === "Region"
			};
		},
		simulator: {
			actors: () => {
				const allowed = (actor) => read(actor, "type") !== "group" && (game.user.isGM || actor.isOwner);
				const scene = (canvas.ready ? canvas.tokens.placeables : []).filter((token) => !!token.actor && allowed(token.actor) && (game.user.isGM || !token.document.hidden)).map((token) => ({
					value: token.actor?.uuid ?? "",
					label: token.document.name,
					group: game.i18n.localize("BNA.UI.Sim.onScene")
				})).sort(byLabel);
				const onScene = new Set(scene.map((option) => option.value));
				const others = listOf(game.actors).filter(isDocument).filter(allowed).filter((actor) => !onScene.has(actor.uuid)).map((actor) => ({
					value: actor.uuid,
					label: actor.name,
					group: game.i18n.localize("BNA.UI.Sim.otherActors")
				})).sort(byLabel);
				return [...new Map(scene.map((option) => [option.value, option])).values(), ...others];
			},
			subscribe: (listener) => {
				const hooks = [
					"createToken",
					...
					"updateToken",
					"canvasReady"
				].map((name) => [name, Hooks.on(name, () => setTimeout(listener, 0))]);
				return () => {
			...
			counterValues: (actorUuid, blueprint) => counterRows(documentFromUuid(actorUuid), blueprint)
		},
		formula: {
...
//#endregion
//#region src/core/legacy/repair.ts
/**
* The checks 2.0.0 wrongly made from v1 filters that were stored with their defaults but never turned on:
* v1 kept `healthPercentages: {value: 50, type: null}` and `tokenSizes: {size, type: null}` on every bonus.
*/
function staleChecks(original) {
	const filters = asRecord(asRecord(original).filters);
	const health = asRecord(filters.healthPercentages);
	const size = asRecord(filters.tokenSizes);
	const stale = [];
	const threshold = asNumber(health.value);
	if (threshold !== null && asNumber(health.type) === null) stale.push({
		id: "c-healthPercentages",
		type: "healthPercentages",
		data: {
			value: threshold,
			type: "atMost"
	...
	const tokenSize = asNumber(size.size);
	if (tokenSize !== null && tokenSize > 0 && asNumber(size.type) === null) stale.push({
		id: "c-tokenSizes",
		type: "tokenSizes",
		data: {
			size: tokenSize,
			type: "atLeast",
			self: asBoolean(size.self)
		}
	});
	return stale;
}
...
function isSame(a, b) {
	return a.id === b.id && a.type === b.type && canonical(a.data) === canonical(b.data);
}
/**
* Undo the stale checks of a blueprint the migration made from `original` (its v1 bonus, kept in the backup).
* Only checks still exactly as the migration made them go; a condition left empty is removed when the migration
* made it and nothing but its "yes" pin leads on. Returns null when nothing changes.
*/
function repairMigratedBlueprint(blueprint, original) {
	if (!asRecord(blueprint.flags["build-n-action"]).migratedFrom) return null;
	const stale = staleChecks(original);
	if (!stale.length) return null;
	let changed = false;
	let nodes = blueprint.nodes.map((node) => {
		if (node.kind !== "condition") return node;
		const checks = Array.isArray(node.data.checks) ? node.data.checks : [];
		const kept = checks.filter((check) => !stale.some((entry) => isSame(entry, check)));
		if (kept.length === checks.length) return node;
		changed = true;
		return {
			...node,
	...
	const condition = nodes.find((node) => node.id === "condition" && node.kind === "condition");
	const incoming = edges.filter((edge) => edge.target === "condition");
	const onlyYes = edges.every((edge) => edge.source !== "condition" || edge.sourcePin === "yes");
	const [from] = incoming;
	const empty = !(condition?.data.checks)?.length && condition?.data.thisItem !== true;
	if (condition && from && incoming.length === 1 && onlyYes && empty) {
		nodes = nodes.filter((node) => node !== condition);
		edges = edges.filter((edge) => edge !== from).map((edge) => edge.source === "condition" ? {
			...edge,
			id: `${from.source}-${from.sourcePin}-${edge.target}`,
			source: from.source,
			sourcePin: from.sourcePin
		} : edge);
	}
	return {
		...blueprint,
...
/** Stored blueprints with migration mistakes undone, using each blueprint's v1 original from the backup. */
function repairStored(flags) {
	const backup = asRecord(flags.legacyBackup);
	let repaired = 0;
	return {
		blueprints: storedEntries(flags.blueprints).map((entry) => {
			const original = backup[storedId(entry)];
			const result = original ? validateBlueprintSource(entry) : null;
			const fixed = result?.ok ? repairMigratedBlueprint(result.value, original) : null;
...
function planMigration(scope, carrier) {
	...
	const changed = converted > 0 || shadowed.length > 0 || repair.repaired > 0;
	return {
		converted,
...
function entryFor(document, plan, error) {
	return {
		uuid: document.uuid,
		label: document.name,
		documentName: document.documentName,
		converted: error ? 0 : plan.converted,
		failed: plan.failed,
		repaired: error ? 0 : plan.repaired,
...
function summarize(entries) {
	return {
		...
		repaired: entries.reduce((sum, entry) => sum + entry.repaired, 0),
...
async function scanMigration(options = {}) {
	return overDocuments(options, false, (documents) => Promise.resolve(summarize(documents.flatMap((document) => {
		...
		return result && (result.converted || result.failed || result.repaired) ? [entryFor(document, result, null)] : [];
	}))));
}
...
async function runMigration(options = {}) {
	return overDocuments(options, true, async (documents) => {
		...
		for (const [index, { document, result }] of pending.entries()) {
			...
			entries.push(entryFor(document, result, error));
	...
	async _renderHTML() {
		...
		if (this.#report && (this.#step === "report" || this.#step === "done")) {
			...
			if (this.#step === "report") {
				...
				if (report.converted || report.repaired) actions.append(button(t("BNA.Migration.run"), () => void this.#run(), true));
			}
...
function simulateBlueprint(blueprint, context) {
	return simulate(context.document, validated(blueprint), {
		...
		saveOutcome: context.saveOutcome ?? "failed",
...
//#endregion
export { resolveCost as $, EVENTS as $t, traitTrees as A, rollDataOf as At, resetCounters as B, createNode as Bt, pendingRoll as C, carrierItem as Ct, effectStatus as D, isDocument as Dt, takeAfterRoll as E, documentFromUuid as Et, resolveForeign as F, enumeratePaths as Ft, sendTrigger as G, randomId as Gt, performOperations as H, defaultsFor as Ht, scaledFormula as I, MODULE_SCOPE as It, costState as J, getCheck as Jt, setSaveReceiver as K, canConnect as Kt, simplifyNumber as L, createBlueprint as Lt, registerAuraPreviews as M, listOf as Mt, modifyFormulaParts as N, read as Nt, loadTraitTrees as O, isSuppressed as Ot, replaceData as P, stringList as Pt, costScales as Q, RESULTS as Qt, resolveModifiers as R, createCheck as Rt, resolveRollTarget as S, actorToken as St, rememberAfterRoll as T, documentBlueprints as Tt, registerSocket as U, isFieldVisible as Ut, resolveCounterIntents as V, duplicateBlueprint as Vt, sendSaveMessage as W, validateBlueprintSource as Wt, costAvailable as X, getResult as Xt, payCost as Y, getEvent as Yt, costOptions as Z, COMMON_RESULT_FIELDS as Zt, guardedAsync as _, clientSetting as _t, removeBlueprint as a, counterDef as an, recordUsage as at, recordUsages as b, setClientSetting as bt, registerMigrationMenu as c, counterValue as cn, buildRollFacts as ct, activationFor as d, asRecord as dn, upstreamEvents as dt, CHECKS as en, sortIntents as et, activationOf as f, asString as fn, runWithSignals as ft, guarded as g, SETTINGS as gt, evaluateEvent as h, EVENT_TYPES as hn, CLIENT_SETTINGS as ht, createApi as i, DEFAULT_COUNTER as in, countRest as it, withinAura as j, touchesBlueprints as jt, sourceLabel as k, originRollData as kt, createFoundryHost as l, nextCounterValue as ln, lintBlueprint as lt, announceApplied as m, CONDITION_NODE_TYPE as mn, openEditor as mt, foundryTranslator as n, STATIC as nn, riderClock as nt, saveBlueprint as o, counterKey as on, createRider as ot, allowIntents as p, asStrings as pn, canEdit as pt, setTriggerReceiver as q, outputPins as qt, createPhraseFormatter as r, COUNTER_RESETS as rn, useRiders as rt, validateIntegrations as s, counterName as sn, matchDisposition as st, foundryChoices as t, CONDITION_NODE_FIELDS as tn, planOperation as tt, registerLibrary as u, asNumber as un, resultEvents as ut, makeRoller as v, registerSettings as vt, registerPending as w, carrierKind as wt, rememberActivation as x, setting as xt, midiActivation as y, resolveEditorTheme as yt, counterStore as z, createEdge as zt };
